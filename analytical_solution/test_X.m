...
[X,Y] = meshgrid(-1:.1:1);
times = linspace(0,1,3);
Z_3D = zeros(length(X), length(X), length(times));

for i = 1:length(times)
    time = times(i);
    Z(:,:,i) = phi_all(X,Y,time,R,A,c,w,z1m);

    % plotting
    figure('Renderer','zbuffer')
    hSurf = surf(X,Y,Z(:,:,i));
    axis tight;    %# fix axis limits
    exportgraphics(gcf,string(i)+'.png','Resolution',600)
end
...
%% Initialize video
myVideo = VideoWriter('all_600dpi'); %open video file
