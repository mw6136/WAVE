...
def step_RK2(num_r, num_theta, phi_prev, phi_current, dr, dtheta, dt, c):
    def f(phi_curr, i, j):
        d2phi_dr2 = (phi_curr[i + 1, j] - 2 * phi_curr[i, j] + phi_curr[i - 1, j]) / (
            dr**2
        )
        d2phi_dtheta2 = (
    ...
    phi_2[2 : num_r - 1, :] = (
        phi_current[2 : num_r - 1, :]
        + (phi_current[2 : num_r - 1, :] - phi_prev[2 : num_r - 1, :]) / 2
        + dt / 4 * k1_v[2 : num_r - 1, :]
    )
    ...
    phi_next[2 : num_r - 1, :] = (
        phi_current[2 : num_r - 1, :]
        + (phi_2[2 : num_r - 1, :] - phi_current[2 : num_r - 1, :]) * 2
        + dt * k2_v[2 : num_r - 1, :]
    )
