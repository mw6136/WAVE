...
def step_bwd(num_r, num_theta, phi_prev, phi_curr, dr, dtheta, dt, c):
    phi_next = phi_curr
    # Update r
    for i in range(2, num_r - 1):
        # Update phi
        for j in range(num_theta):
            def equation(phi_next_val):
                # Spatial derivatives
                ...
                # Define a function for backward Euler equation
            
                return phi_next_val - 2*phi_curr[i, j] + phi_prev[i, j] - dt ** 2 * d2phi_dt2

            # Use a root finder to solve for phi_next_val
            phi_next_val = newton(equation, phi_curr[i, j])
...
def bwd_euler(R, A, omega, c, tmax, num_timesteps, num_r, num_theta):
    ...
    for n in tqdm(range(1, num_timesteps + 1)):
        ...
        phi_curr, phi_next = step_bwd(num_r, num_theta, phi_prev, phi_curr, dr, dtheta, dt, c)
